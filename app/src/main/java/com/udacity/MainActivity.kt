...
class MainActivity : AppCompatActivity() {
    ...
    private var selectedOption: RadioButtonOptions? = null
            ...
            if (download_radio_group.checkedRadioButtonId != -1) {
                when (download_radio_group.checkedRadioButtonId) {
                    glide_radioButton.id -> {
                        selectedOption = RadioButtonOptions.GLIDE
                    }
                    load_app_radioButton.id -> {
                        selectedOption = RadioButtonOptions.UDACITY
                    }
                    retrofit_radioButton.id -> {
                        selectedOption = RadioButtonOptions.RETROFIT
                    }
                }
                ...
                Toast.makeText(this, getString(R.string.invalid_option_text), Toast.LENGTH_SHORT)
                    .show()
            }
        }
    }
    ...
    private fun sendNotifications(success: Boolean, radioButtonOptions: RadioButtonOptions) {
    ...
    private fun download(selectedOption: RadioButtonOptions?) {
        val request =
